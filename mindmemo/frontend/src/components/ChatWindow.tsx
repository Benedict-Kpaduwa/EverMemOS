...
export default function ChatWindow({ onMessageSent }: ChatWindowProps) {
  ...
  const autoResize = () => {
    ...
    el.style.height = Math.min(el.scrollHeight, 140) + "px"; // Slightly smaller max-height for mobile
  };
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      setSending(false);
      // Only auto-focus on non-touch devices to avoid annoying mobile keyboard popups immediately
  ...
  const handleKey = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "Enter" && !e.shiftKey) { 
      // Prevent desktop enter from firing instantly if on mobile sometimes, but standard is fine
      e.preventDefault(); 
      sendMessage(); 
    }
  };
        ...
        <div className="max-w-3xl mx-auto w-full relative">
          ...
          <div className={`absolute -top-12 left-0 right-0 flex justify-center transition-all duration-500 pointer-events-none ${savingMemory ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="bg-emerald-500/15 backdrop-blur-md border border-emerald-500/30 px-4 py-1.5 rounded-full flex items-center gap-2 shadow-lg shadow-emerald-500/10">
              <svg className="animate-spin h-3.5 w-3.5 text-emerald-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
              <span className="text-[11px] font-semibold text-emerald-300 tracking-wide uppercase mt-px">Syncing to memory</span>
            </div>
          </div>
            ...
            <div className="relative flex items-end gap-2 sm:gap-3 glass-card bg-zinc-900/60 rounded-[20px] sm:rounded-3xl p-1.5 sm:p-2.5 border border-white/10 sm:border-white/5">
              ...
              <div className="flex items-center gap-2 sm:gap-3 pr-1 sm:pr-2 pb-1 flex-shrink-0">
                ...
                <button
                  ...
                  className={`
                    w-9 h-9 sm:w-11 sm:h-11 rounded-full sm:rounded-2xl flex items-center justify-center transition-all duration-300 flex-shrink-0
                    ${input.trim() && !sending 
                      ? 'bg-emerald-500 hover:bg-emerald-400 text-white shadow-lg shadow-emerald-500/25 sm:hover:-translate-y-0.5' 
                      : 'bg-zinc-800/80 text-zinc-500 cursor-not-allowed'}
                  `}
                >
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={input.trim() && !sending ? 'translate-x-[1px] sm:-translate-y-[1px] w-4 h-4 sm:w-5 sm:h-5' : 'w-4 h-4 sm:w-5 sm:h-5'}>
          ...
          <div className="flex justify-between items-center mt-2 px-3 opacity-80 sm:opacity-100">
